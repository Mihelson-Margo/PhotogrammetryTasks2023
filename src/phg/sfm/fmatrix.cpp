...
//
        ...
        Eigen::VectorXd null_space(9);
        for (int i = 0; i < 9; i++) null_space[i] = svda.matrixV()(i, 8);
...
//
        ...
        s(0, 0) = svdf.singularValues()[0];
        s(1, 1) = svdf.singularValues()[1];
        //s(2, 2) = svdf.singularValues()[2];
        //for (int i = 0; i < 3; i++)
        //    std::cout << "SV = " << svdf.singularValues()[i] << "\n";

        cv::Matx33d v1(3, 3), v(3, 3), u(3, 3);
        for (int i = 0 ; i <3; i++) {
            for (int j = 0; j < 3; j++) {
                v1(i, j) = svdf.matrixV()(i, j);
                u(i, j) = svdf.matrixU()(i, j);
            }
        }
        cv::transpose(v1, v);
        cv::Matx33d Fcv = u * s * v;
        //copy(F, Fcv);
        return Fcv;
    }
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        for (int i = 0; i < 2; i++) {
            double mean = 0;
            for (int j = 0; j < (int)m.size(); j++) mean += m[j][i];
            mean /= m.size();
            double sigma = 0;
            for (int j = 0; j < (int)m.size(); j++)
                sigma += (m[j][i] - mean)*(m[j][i] - mean);
            sigma = std::sqrt(sigma / m.size());
            double c = std::sqrt(2) / sigma;

            transform(i, i) = c;
            transform((i + 1)%2, i) = 0;
            transform(3, i) = - c * mean;
        }
        return transform;
    }
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
        ...
        const int n_samples = 8; //TODO: check it
        uint64_t seed = 1;
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            int support = 0;
            for (int i = 0; i < n_matches; ++i) {
                if (phg::epipolarTest(m0[i], m1[i], F, threshold_px) && phg::epipolarTest(m1[i], m0[i], F1, threshold_px))
                {
                    ++support;
                }
            }
