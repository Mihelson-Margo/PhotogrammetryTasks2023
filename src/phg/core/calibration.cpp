...
cv::Vec3d phg::Calibration::unproject(const cv::Vec2d &pixel) const
{
    ...
    x -= k1_ * r + k2_ * r * r;
    y -= k1_ * r + k2_ * r * r;
    // Вообще говоря мы тут радиус посчитали у искаженной точки, а когда применяли
    // прямое преобразование радиус, наоборот, считался у точки до искажения.
    // Но решать уравнения 4-й степени на 2 неизвестных как-то не хочется
