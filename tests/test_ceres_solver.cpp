...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Ответ: потому что мы посчитали производную функции f(x) = 10 - x, и она -1
    // а минимизируем мы f(x)^2, и ее производная это 2 f'f = -2f, то есть 0 в точке х = 5
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // DONE: бага в расстоянии до параболоида найдена
    }
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        residual[0] = (line[0] * samplePoint[0] + line[1] * samplePoint[1] + line[2])
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    float scale_ideal = ideal_line[2];
    float scale_our = line_params[2];
    for (int d = 0; d < 3; ++d) {
        ASSERT_NEAR(line_params[d]/scale_our, ideal_line[d]/scale_ideal, threshold);
        ...
        // Ответ: у прямой две степени свободы, а мы ее закодировали тремя параметрами, отсюда неоднозначность
        // проще всего как-нибудь нормировать коэффициенты a, b, c
        // мне больше всего нравится идея нормировать на sqrt(a*a + b*b),
        // потому что единичная нормаль -- это всегда приятно, тем более расстояние
        // будет считаться проще. Ну и потому что a*a + b*b > 0 всегда
        // Но с такой нормировкой прилетали проблемы с точностью на assert
        // так что пришлось закостылять :(
        //
        // По-хорошему надо конечно задавать прямую двумя параметрами,
        // например по двум точкам: (x - x1)/(x2 - x1) = (y - y1)/(y2 - y1)
    }
    ...
    // TODO 8 раскоментируйте, почему эта проверка падает? как поправить?
    // Ответ: если данные шумные, странно требовать чтобы истинная прямая
    // проходила вообще через все точки
    ASSERT_LT(mse, 1.1 * sigma * sigma);
    // TODO 9 раскомментируйте, почему проверка падает? на каких тестах она падает, на каких проходит? попробуйте отладить рассчет mse_inliers_distance в evaluateLine
    // Ответ: падало на тестах с шумными данными, потому что расстояние до прямой
    // знаковое, и надо проверять что его модуль меньше 3*сигма
